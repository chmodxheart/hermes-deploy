...
| Kubernetes resources | Flux/Talos | Current external Kubernetes/Talos source | Phase 3 defines the in-repo representation and integration method. | Talos cluster and Flux GitOps source | Flux owns Kubernetes workloads that remain in-cluster; do not bypass Flux for durable cluster-owned changes. |
| SOPS/age and platform secrets | Platform-specific secret owners | `nixos/secrets/*.yaml`, `.sops.yaml`, ignored local stores, and external secret sources | Imported Home Manager and Kubernetes/Talos material must preserve encrypted boundaries. | SOPS/age, Flux deploy keys, Talos secrets, Terraform variable stores | Decrypted SOPS/age material, Flux deploy keys, Talos secrets, Terraform variables, private credentials, and generated credential material must not be committed as tracked plaintext. |
